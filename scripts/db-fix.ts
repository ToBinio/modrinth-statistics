...
const data1 = db.projectStatistics
	.find({ key: new RegExp(dateSuffix1 + "$") })
	.toArray();
const data2 = db.projectStatistics
	.find({ key: new RegExp(dateSuffix2 + "$") })
	.toArray();
	...
	if (entry1 && entry2) {
		...
		for (let j = 0; j < data1.data.length; j++) {
			...
			for (let k = 0; k < value1.values.length; k++) {
				...
				something2.downloads = Math.round(
					(something1.downloads + something2.downloads) / 2,
				);
				something2.versions = Math.round(
					(something1.versions + something2.versions) / 2,
				);
				something2.count = Math.round(
					(something1.count + something2.count) / 2,
				);
			}
		}
		...
		print(
			`Inserted interpolated entry for ${newEntry.key} ${JSON.stringify(newEntry)}`,
		);
	} else {
...
const data1 = db.globalStatistics
	.find({ key: new RegExp(dateSuffix1 + "$") })
	.toArray();
const data2 = db.globalStatistics
	.find({ key: new RegExp(dateSuffix2 + "$") })
	.toArray();
